...
export async function reloadFunc() {
  ...
  // Note: No simulation.forward() call here since we're not simulating

  // Update joint drag manager with new simulation
  if (this.jointDragManager) {
    this.jointDragManager.simulation = this.simulation;
  }
...
/** @param {MuJoCoViewer} parentContext*/
export function setupGUI(parentContext) {
  ...
  // Only add keyframe input if the model has keyframes
  let nkeys = parentContext.model.nkey;
  let keyframeGUI = null;
  if (nkeys > 0) {
    // Add a numeric input for keyframeNumber (no slider)
    keyframeGUI = parentContext.gui
      .add(parentContext.params, "keyframeNumber")
      .name("Load Keyframe")
      .listen();
    keyframeGUI.onChange((value) => {
      // Clamp value to valid range
      let keyIdx = Math.max(0, Math.min(nkeys - 1, Math.floor(value)));
      if (keyIdx < parentContext.model.nkey) {
        parentContext.simulation.qpos.set(
          parentContext.model.key_qpos.slice(
            keyIdx * parentContext.model.nq,
            (keyIdx + 1) * parentContext.model.nq
          )
        );
        // Update the scene to reflect the new pose
  ...
  // Add body dragging controls
  let jointControlsFolder = parentContext.gui.addFolder("Body Controls");

  jointControlsFolder
    .add(
      {
        resetPose: () => {
          if (parentContext.simulation) {
    ...
    .add(
      {
        help: () => {
          alert(
            "Body Dragging Instructions:\n\n" +
              "• Click and drag on any body part to move it\n" +
              "• Double-click to highlight/select a body part\n" +
              "• Only one body can be moved at a time\n" +
              "• Bodies follow the cursor position directly\n" +
              "• Use 'Reset Pose' to return to initial position\n" +
              "• Camera controls are disabled while dragging"
          );
        },
      },
      "help"
    )
    .name("Body Dragging Help");

  jointControlsFolder.open();
 ...
 * @param {MuJoCoViewer} parent The three.js Scene Object to add the MuJoCo model elements to
 */
