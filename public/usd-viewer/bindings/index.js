...
/**
 * @type {Promise<import("../index.js").USD> | null}
 */
let usd_module_promise = null;

/**
 * @param {undefined | import("../index.js").GetUsdModuleOptions} opts
 */
export async function getUsdModule(opts) {
  ...
  /**
   * @type {import("../index.js").getUsdModule}
   */
  const getUsdModuleFn = globalThis["NEEDLE:USD:GET"];

  if (!getUsdModuleFn) {
    throw new Error(
      '"NEEDLE:USD:GET" not found in globalThis - please modify "emHdBindings.js" and add: globalThis["NEEDLE:USD:GET"] = getUsdModule;'
    );
  }
    ...
    getPreloadedPackage(name, size) {
      ...
      if (name.includes("emHdBindings.data")) {
        if (preloaded_data.byteLength !== size) {
          throw new Error(
            `emHdBindings.data size mismatch: expected ${size} but got ${preloaded_data.byteLength}\n${data.default}`
          );
        }
      }
      return null;
