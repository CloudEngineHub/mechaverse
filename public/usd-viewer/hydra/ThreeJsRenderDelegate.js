...
class TextureRegistry {
  ...
  getTexture(resourcePath) {
    ...
    if (lowercaseFilename.indexOf(".png") >= lowercaseFilename.length - 5) {
      ...
      lowercaseFilename.indexOf(".jpg") >=
      lowercaseFilename.length - 5
    ) {
      filetype = "image/jpeg";
    } else if (
      lowercaseFilename.indexOf(".jpeg") >=
      lowercaseFilename.length - 5
    ) {
      filetype = "image/jpeg";
    } else if (
      lowercaseFilename.indexOf(".exr") >=
      lowercaseFilename.length - 4
    ) {
      console.warn("EXR textures are not fully supported yet", resourcePath);
      // using EXRLoader explicitly
      filetype = "image/x-exr";
    } else if (
      lowercaseFilename.indexOf(".tga") >=
      lowercaseFilename.length - 4
    ) {
...
class HydraMesh {
  ...
  updateOrder(attribute, attributeName, dimension = 3) {
    if (debugMeshes)
      console.log("updateOrder", attribute, attributeName, dimension);
  ...
  setMaterial(materialId) {
    if (debugMaterials)
      console.log(
        "Setting material on hydra prim: " + materialId,
        this._mesh,
        materialId,
        this._interface.materials[materialId]
      );
  ...
  setDisplayColor(data, interpolation) {
    ...
    if (interpolation === "constant") {
      ...
      if (warningMessagesToCount.has(interpolation)) {
        warningMessagesToCount.set(
          interpolation,
          warningMessagesToCount.get(interpolation) + 1
        );
      } else {
    ...
    switch (name) {
      ...
      default:
        if (warningMessagesToCount.has(name)) {
          warningMessagesToCount.set(
            name,
            warningMessagesToCount.get(name) + 1
          );
        } else {
...
class HydraMaterial {
  ...
  constructor(id, hydraInterface) {
    ...
    if (disableMaterials) defaultMaterial.color = new Color(0x999999);
  ...
  updateNode(networkId, path, parameters) {
    if (debugTextures)
      console.log(
        "Updating Material Node: " + networkId + " " + path,
        parameters
      );
  ...
  async updateFinished(type, relationships) {
    ...
    let _name = this._id;
    let lastSlash = _name.lastIndexOf("/");
    if (lastSlash >= 0) _name = _name.substring(lastSlash + 1);
    this._material.name = _name;

    // Assign textures
    const haveRoughnessMap = !!(
      mainMaterialNode.roughness && mainMaterialNode.roughness.nodeIn
    );
    const haveMetalnessMap = !!(
      mainMaterialNode.metallic && mainMaterialNode.metallic.nodeIn
    );
    const haveOcclusionMap = !!(
      mainMaterialNode.occlusion && mainMaterialNode.occlusion.nodeIn
    );
    ...
    if (!disableTextures) {
      ...
      if (haveRoughnessMap && !haveMetalnessMap) {
        ...
        if (this._material.metalnessMap)
          ...
          console.error(
            "Something went wrong with the texture promise; haveRoughnessMap is true but no roughnessMap was loaded."
          );
    ...
    if (debugMaterials)
      console.log(
        'Material Node "' + this._material.name + '"',
        mainMaterialNode,
        "Resulting Material",
        this._material
      );
